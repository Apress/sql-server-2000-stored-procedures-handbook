...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[sp__procedure$decrypt]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp__procedure$decrypt]
GO

CREATE PROCEDURE sp__procedure$decrypt  
...
-- It will rollback the changes in a transaction so it shouldn't be too bad, but there
-- will be a short time in the middle where the procedure will not be itself.
-- ---------------------------------------------------------------------------
--	Description
--	This procedure will decrypt a stored procedure in SQL 2000.	
--
--	Implementation Notes
--	This procedure uses UNDOCUMENTED functionality. syscomments.encrypted column
--
--	Results Sets - unencrypted text of an encrypted stored procedure
--		
--	Returns
--
--	Input: 	@procedure - procedure name
--			@revfl - safety bar to make sure you want to run this procedure
--
--	Preconditions
--	
--	Postconditions
...
--	charles hawkins 1/24/2003		1.1		Wrapped all alterations of existing procedure
--											in rolled-back transaction so nothing ultimately
--											changes - Louis Davidson recommendation.
-- -----------------------------------------------------------------------------------
AS
SET NOCOUNT ON
